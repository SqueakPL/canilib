...
CANI_EXPORT CANI_CHAR CANI_FastResize(CANI_BGRAQUAD* src_adress, CANI_LONG src_cx, CANI_LONG src_cy, CANI_BGRAQUAD* dest_adress, CANI_LONG dest_cx, CANI_LONG dest_cy) {
	...
	if (src_cx == dest_cx && src_cy == dest_cy) {
		...
		CANI_MEMCPY(dest_adress, src_adress,
			src_cx * src_cy * sizeof(CANI_BGRAQUAD));
		return TRUE;
	}

	buffor = (CANI_BGRAQUAD*)CANI_MALLOC((src_cy * dest_cx) * sizeof(CANI_BGRAQUAD));//new CANI_BGRAQUAD[src_cy * dest_cx];

	if (buffor) {
		CANI_LONG x, y, x2, y2;

		if (dest_cx > src_cx) {
			CANI_BGRAQUAD* dest_now = buffor;

			for (y = 0; y < src_cy; y++) {
				for (x = 0; x < dest_cx; x++) {

					CANI_BGRAQUAD* src;
					CANI_BGRAQUAD* dest = dest_now;//&buffor[ y * dest_cx + x ];
					...
					//CANI_MEMCPY( dest, src, sizeof(CANI_BGRAQUAD) );
			...
			CANI_MEMCPY(buffor, src_adress,
				src_cy * dest_cx * sizeof(CANI_BGRAQUAD));
		}


		if (dest_cy > src_cy) {

			CANI_BGRAQUAD* dest_now = dest_adress;//[ y * dest_cx + x ];

			for (y = 0; y < dest_cy; y++) {
				for (x = 0; x < dest_cx; x++) {

					CANI_BGRAQUAD* src;
					//CANI_BGRAQUAD *dest = dest_now;//&dest_adress[ y * dest_cx + x ];
			...
			CANI_MEMCPY(dest_adress, buffor, dest_cy * dest_cx * sizeof(CANI_BGRAQUAD));
		}
		...
		if (dest_cx > src_cx) {
			for (y = 0; y < src_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					CANI_BGRAQUAD* dest = &buffor[y * dest_cx + x];
					...
					if (reszta > 0 && wynik >= 0 && (zaokraglenie < src_cx)) {
						...
						buffor[y * dest_cx + x].Red = (unsigned char)wartosc;
						...
						buffor[y * dest_cx + x].Green = (unsigned char)wartosc;
						...
						buffor[y * dest_cx + x].Blue = (unsigned char)wartosc;
						...
						buffor[y * dest_cx + x].Alpha = (unsigned char)wartosc;
			...
			CANI_MEMCPY(buffor, src_adress, src_cy * dest_cx * sizeof(CANI_BGRAQUAD));
		}
		...
		if (dest_cy > src_cy) {
			for (y = 0; y < dest_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					CANI_BGRAQUAD* dest = &dest_adress[y * dest_cx + x];
					...
					if (reszta > 0 && wynik >= 0 && (zaokraglenie < src_cx)) {
						...
						dest_adress[y * dest_cx + x].Red = (unsigned char)wartosc;
						...
						dest_adress[y * dest_cx + x].Green = (unsigned char)wartosc;
						...
						dest_adress[y * dest_cx + x].Blue = (unsigned char)wartosc;
						...
						dest_adress[y * dest_cx + x].Alpha = (unsigned char)wartosc;
...
CANI_DOUBLE CANI_LagrangeInterpolation(
		...
		if (dest_cx != src_cx) {
			...
			for (y = 0; y < src_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					wynik = CANI_LagrangeInterpolation(x4, y4, 4, x5);
		...
		if (dest_cy != src_cy) {
			...
			for (y = 0; y < dest_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					wynik = CANI_LagrangeInterpolation(x4, y4, 4, y5);
