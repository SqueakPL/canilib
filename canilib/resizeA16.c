...
CANI_EXPORT CANI_CHAR CANI_FastResizeAlpha16(CANI_WORD* src_adress, CANI_LONG src_cx, CANI_LONG src_cy, CANI_WORD* dest_adress, CANI_LONG dest_cx, CANI_LONG dest_cy) {
	...
	if (src_cx == dest_cx && src_cy == dest_cy) {
		...
		CANI_MEMCPY(dest_adress, src_adress,
			src_cx * src_cy * sizeof(CANI_WORD));
		return TRUE;
	}

	buffor = (CANI_WORD*)CANI_MALLOC((src_cy * dest_cx) * sizeof(CANI_WORD));//new CANI_WORD[src_cy * dest_cx];

	if (buffor) {
		CANI_LONG x, y, x2, y2;

		if (dest_cx > src_cx) {
			CANI_WORD* dest_now = buffor;

			for (y = 0; y < src_cy; y++) {
				for (x = 0; x < dest_cx; x++) {

					CANI_WORD* src;
					CANI_WORD* dest = dest_now;//&buffor[ y * dest_cx + x ];
					...
					CANI_MEMCPY(dest, src, sizeof(CANI_WORD));
			...
			CANI_MEMCPY(buffor, src_adress,
				src_cy * dest_cx * sizeof(CANI_WORD));
		}


		if (dest_cy > src_cy) {

			CANI_WORD* dest_now = dest_adress;//[ y * dest_cx + x ];

			for (y = 0; y < dest_cy; y++) {
				for (x = 0; x < dest_cx; x++) {

					CANI_WORD* src;
					//CANI_WORD *dest = dest_now;//&dest_adress[ y * dest_cx + x ];
					...
					CANI_MEMCPY(dest_now, src, sizeof(CANI_WORD));
			...
			CANI_MEMCPY(dest_adress, buffor, dest_cy * dest_cx * sizeof(CANI_WORD));
		}
		...
		if (dest_cx > src_cx) {
			for (y = 0; y < src_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					CANI_WORD* dest = &buffor[y * dest_cx + x];
					...
					if (reszta > 0 && wynik >= 0 && (zaokraglenie < src_cx)) {
						...
						buffor[y * dest_cx + x] = (CANI_WORD)wartosc;
			...
			CANI_MEMCPY(buffor, src_adress, src_cy * dest_cx * sizeof(CANI_WORD));
		}
		...
		if (dest_cy > src_cy) {
			for (y = 0; y < dest_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					CANI_WORD* dest = &dest_adress[y * dest_cx + x];
					...
					if (reszta > 0 && wynik >= 0 && (zaokraglenie < src_cx)) {
						...
						dest_adress[y * dest_cx + x] = (CANI_WORD)wartosc;
...
CANI_DOUBLE CANI_LagrangeInterpolationAlpha16(
		...
		if (dest_cx != src_cx) {
			...
			for (y = 0; y < src_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					wynik = CANI_LagrangeInterpolationAlpha16(x4, y4, 4, x5);
		...
		if (dest_cy != src_cy) {
			...
			for (y = 0; y < dest_cy; y++) {
				for (x = 0; x < dest_cx; x++) {
					...
					wynik = CANI_LagrangeInterpolationAlpha16(x4, y4, 4, y5);
